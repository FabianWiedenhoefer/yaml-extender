...
@mock.patch('yaml_extender.yaml_loader.load')
@mock.patch('pathlib.Path.is_file')
def test_parameter_include_and_inline_for_loop(is_file_mock, load_func):
    is_file_mock.return_value = True
    content = yaml.safe_load("""
dict_1:
  subvalue_1: abc
...
""")
    ...
    dict_1:
      subvalue_1: abc
      subvalue_2: xyz
      subvalue_3: "{{xyml.for:i:array_1:{{i}}}}"
    """)
    load_func.return_value = {"subvalue_2": "{{subvalue_2}}", "subvalue_3": "{{xyml.for:i:array_1:{{i}}}}"}
    inc_resolver = IncludeResolver()
    result = inc_resolver.resolve(content)
